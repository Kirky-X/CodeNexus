...
fn main() {
    println!("=== CodeNexus Example: Call Tracing ===\n");

    let ctx = setup(SAMPLE_CODE);
    let _result = index_sample_code(&ctx, "trace-demo");
    ...
    match facade.trace("main", TraceType::Calls, 5) {
        Ok(result) => print_trace_result(&result),
        Err(e) => println!("  Error: {e}\n"),
    }

    println!("--- Forward call trace: transform (depth=3) ---");
    let facade = TraceFacade::new(&graph);
    match facade.trace("transform", TraceType::Calls, 3) {
        Ok(result) => print_trace_result(&result),
        Err(e) => println!("  Error: {e}\n"),
    }
...
fn print_trace_result(result: &codenexus::trace::TraceResult) {
