...
impl Default for FortlsClient {
    fn default() -> Self {
        Self::new()
    }
}

impl LspProvider for FortlsClient {
...
mod tests {
    ...
    fn shutdown_without_start_returns_ok() {
        assert!(FortlsClient::new().shutdown().is_ok());
    }
    ...
    fn query_without_start_returns_communication_error() {
        let client = FortlsClient::new();
        assert!(matches!(client.definition(Path::new("/tmp/test.f90"), 0, 0), Err(LspError::Communication(_))));
        assert!(matches!(client.hover(Path::new("/tmp/test.f90"), 0, 0), Err(LspError::Communication(_))));
    }

    #[test]
    fn new_uses_default_server_path() {
        assert_eq!(FortlsClient::new().server_path, PathBuf::from(DEFAULT_SERVER_PATH));
    }

    #[test]
    fn with_server_path_overrides_default() {
        let p = PathBuf::from("/custom/fortls");
        assert_eq!(FortlsClient::with_server_path(p.clone()).server_path, p);
    }
    ...
    fn hp() -> HoverParams { HoverParams { text_document_position_params: TextDocumentPositionParams { text_document: TextDocumentIdentifier { uri: Url::parse("file:///tmp/x.f90").unwrap() }, position: Position { line: 0, character: 0 } }, work_done_progress_params: WorkDoneProgressParams::default() } }
    ...
    fn loc() -> lsp_types::Location { lsp_types::Location { uri: Url::parse("file:///tmp/test.f90").unwrap(), range: lsp_types::Range { start: Position { line: 5, character: 10 }, end: Position { line: 5, character: 20 } } } }
    ...
    fn definition_with_mock() {
        ...
        *c.session.lock().unwrap() = Some(s);
        ...
        assert_eq!(c.definition(Path::new("/tmp/test.f90"), 0, 0).unwrap().unwrap().range.start.line, 5);
        let _ = c.shutdown();
    }
    ...
    fn hover_with_mock() {
        ...
        let c = FortlsClient::new();
        *c.session.lock().unwrap() = Some(s);
        let hover = lsp_types::Hover { contents: lsp_types::HoverContents::Markup(lsp_types::MarkupContent { kind: lsp_types::MarkupKind::Markdown, value: "integer function foo()".into() }), range: None };
        rt.send(Message::Response(Response { id: RequestId::from(1), result: Some(serde_json::to_value(hover).unwrap()), error: None })).unwrap();
        assert!(c.hover(Path::new("/tmp/test.f90"), 0, 0).unwrap().is_some());
        let _ = c.shutdown();
    }
    ...
    fn integration_start_shutdown() {
        if std::process::Command::new("fortls").arg("--version").stdout(std::process::Stdio::null()).stderr(std::process::Stdio::null()).status().is_err() { return; }
        let ws = tempfile::TempDir::new().unwrap();
        std::fs::write(ws.path().join("test.f90"), "program test\nend program test\n").unwrap();
        let c = FortlsClient::new();
