...
mod tests {
    ...
    fn query_without_start_returns_communication_error() {
        ...
        assert!(matches!(
            client.definition(Path::new("/tmp/Test.java"), 0, 0),
            Err(LspError::Communication(_))
        ));
        assert!(matches!(
            client.hover(Path::new("/tmp/Test.java"), 0, 0),
            Err(LspError::Communication(_))
        ));
    }
        ...
        HoverParams {
            text_document_position_params: TextDocumentPositionParams {
                text_document: TextDocumentIdentifier {
                    uri: "file:///tmp/x.java".parse::<Uri>().unwrap(),
                },
                position: Position {
                    line: 0,
                    character: 0,
    ...
    fn loc() -> lsp_types::Location {
        lsp_types::Location {
            uri: "file:///tmp/Test.java".parse::<Uri>().unwrap(),
    ...
    fn definition_with_mock() {
        ...
        *c.session.lock().unwrap() = Some(s);
        ...
        .unwrap();
        assert_eq!(
            c.definition(Path::new("/tmp/Test.java"), 0, 0)
                .unwrap()
                .unwrap()
                .range
                .start
    ...
    fn hover_with_mock() {
        ...
        *c.session.lock().unwrap() = Some(s);
        ...
        .unwrap();
        assert!(c
            .hover(Path::new("/tmp/Test.java"), 0, 0)
            .unwrap()
            .is_some());
        let _ = c.shutdown();
    }
