...
**CodeNexus** 将源代码仓库索引为可查询的知识图谱：tree-sitter 多语言解析、LadybugDB 图存储，支持 Cypher 查询、调用链追踪、影响分析、语义搜索与多智能体 MCP 集成。本指南从安装入门一路走到全部 37 个子命令（外加 `mcp` 服务模式）的用法、复杂度分析与死代码检测详解，以及故障排查。
