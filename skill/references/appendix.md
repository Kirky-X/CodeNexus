...
| axum route extraction | P3 (open) | `route_map`, `shape_check`, `api_impact` (Rust axum) | `extract_axum_routes` only recognizes bare-identifier handlers (`get(handler)`); path-qualified (`get(crate::module::handler)`) and closure (`get(\|\| { ... })`) handlers are skipped. Manually verify handler coverage for axum projects using path-qualified or closure handlers. |
