...
| axum route extraction | P3 (open) | `route_map`, `shape_check`, `cross_service`, `api_impact` (Rust axum) | `extract_axum_routes` only recognizes bare-identifier handlers (`get(handler)`); path-qualified (`get(crate::module::handler)`) and closure (`get(\|\| { ... })`) handlers are skipped. For axum projects using `Router::new().route(...)` macro registration, route strings are macro arguments rather than standalone string literals, so `cross_service`/`route_map`/`api_impact` return empty `[]`. Manually verify handler coverage for axum projects. |
