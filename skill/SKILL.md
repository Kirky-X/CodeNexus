...
- **`impact` on very large graphs** — two different node sets in the output: the true blast-radius (`affected`) is capped by `trace_upstream` at `MAX_NODES_LIMIT=1000` nodes + `max_depth≤10`, so the traversal itself cannot run away; BUT `nodes`/`edges`/`node_count` echo the **entire subgraph** loaded by `load_graph` (Cypher has no `LIMIT`), and those are NOT subject to the 1000 cap and get fully JSON-serialized. That load + serialization is the real cost. Verified 2026-07-17 on a self-indexed CodeNexus graph (9232 symbols): a **leaf** symbol (`analyze`, `--depth 10`) returns in **~4.5s** (`node_count` small); an **entry-chain** symbol (`sanitize_project_name`, depended on by `run_cli`, `--depth 3`) fans `node_count` to **5034** and returns in **~77s** — both exit 0, well under the 120s tool timeout. For hot targets: narrow `--edge_types` / lower `--depth` (it sets `load_depth = depth.max(max_depth)`) / pick a leaf.
